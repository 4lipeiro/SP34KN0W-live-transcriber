...
class DeepgramTranscriber:
    ...
    def __init__(self, api_key, language="it", ui=None, session_name=None, model="nova-2", translate=False, mic_device=None):
        ...
        # ── local MT initialisation ──────────────────────────────
        if self.translate:
            device_id = 0 if torch.cuda.is_available() else -1
            self.translator = pipeline(
                task="translation_it_to_en",
                model="Helsinki-NLP/opus-mt-it-en",
                device=device_id,            # -1 = CPU
                batch_size=8,                # keeps latency low
            )
            print("device id 0 for gpu and -1 for cpu, current device id is:" , device_id)
    ...
    def _on_transcript(self, *args, **kwargs):
        ...
        try:
            ...
            if 'result' in kwargs:
                ...
                # translation logic
                translation = None
                if self.translate and is_final:
                    # One-liner translation, returns list[dict]
    ...
    async def _save_transcript(self, append_mode=False):
        ...
        try:
            ...
            # ── save pure-translation file ─────────────────────────
            # if self.translate:
            #     trans_path = os.path.join(self.sessions_dir, f"{self.session_name}_en.md")
            #     with open(trans_path, 'w', encoding='utf-8') as tf:
            #         tf.write(f"# English Translation of {self.session_name}\n\n")
            #         for entry in self.transcript_data:
            #             if "translation" in entry:
            #                 tf.write(f"{entry['translation']}\n\n")
            #     logger.info(f"Translation saved to {trans_path}")
                        
                        
            if self.translate:
                out_en = os.path.join(self.sessions_dir, f"{self.session_name}_en.md")
                with open(out_en, "w", encoding="utf-8") as f2:
                    f2.write(f"# English Translation of {self.session_name}\n\n")
                    for e in self.transcript_data:
                        if "translation" in e and e["translation"]:
                            f2.write(f"{e['translation']}\n\n")
                logger.info(f"Saved translation file: {out_en}")
